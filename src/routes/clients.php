...
include_once  dirname(__DIR__).'/entities/Clients.php';
include_once  dirname(__DIR__).'/entities/ProuserClients.php';
...
$container['logger'] = function ($c) {
    ...
    $log->pushHandler(new StreamHandler(__DIR__ . '/../logs/clients.log', Logger::INFO));
...
$app->group('/clients', function () use ($app) {
    ...
    $app->get('/client/{id}/', function (Request $request, Response $response) {
        ...
        $user = new Clients($id, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null);
     ...
     * This method list the latest published messages
     */
    $app->get('/list/{pro_user}/', function (Request $request, Response $response) {
        $pro = $request->getAttribute("pro_user");
        $client = new ProUsers(null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null);
        // Gets the database connection
        $conn = PDOConnection::getConnection();
        $data = $client->clientsByPro($conn,$pro);
     ...
     */
    $app->delete('/pro_user/delete/', function (Request $request, Response $response) {

        $client = $request->getParam("client");
        $pro = $request->getParam("pro_user");
        $pc = new ProuserClients(null, $pro, $client, null, null);
        // Gets the database connection
        $conn = PDOConnection::getConnection();
        $data = $pc->deleteLink($conn);
    ...
    $app->get('/login/{email}/{password}/', function (Request $request, Response $response) {
        ...
        if($email == null | $password == null){
            ...
            $client = new Clients(null, $email, null, $email, null, null, null, null, null, null, null, null, null, null, null, null, null, $password, null, null);
            // Gets the database connection
            $conn = PDOConnection::getConnection();

            $data = $client->login($conn);
            if($data[0]=="sucess"){
                $client->userByEmail($conn);
                $desc = "login";
                $table = "Clients";
                $action_id = $client->getId();
    ...
    $app->post('/add/', function (Request $request, Response $response) {
        ...
        if( $email == null | $password == null){
            ...
            $client = new Clients(null, $email, 'not assigned', $username, $genre, $prenom, $nom, $date_naissance, $comp_adresse, $code_postal, $tel, $ind_tel, $mobile, $ind_mobile, $date_creation, $longitude, $latitude, $password, 1, $date_creation);
     ...
     * This method adds a new client
     */
    $app->post('/pro_user/add/', function (Request $request, Response $response) {
        // Unique ID
        //$guid = uniqid();
        ...
        if( $email == null | $password == null){
            ...
            $client->userByEmail($conn);
            $pc = new ProuserClients(null, $pro, $client->getId(), null, null);
            $x = $pc->addLink($conn);
